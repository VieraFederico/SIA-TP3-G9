...
def tanh_perceptron_test():
    ...
    Y = np.tanh(X)
    perceptron = PerceptronNonLinear(learning_rate=0.15, epochs=30)
    ...
    plot_adaline_regression(
        X, Y, perceptron,
        f"TANH Regression (Beta = 0.1) Result",
        f"tanh_result.png",
        xlim=(-6, 6),
        ylim=(-1.5, 1.5),
        centered=True
...
def perceptron_linear_test():
    ...
    X_linear_test = np.linspace(-5, 10, 50)
    Y_linear_test = 2 * X_linear_test + 5
    print("Predictions:", p_linear.predict(X_linear_test.reshape(-1, 1)))


    plot_adaline_regression(
        X_linear_test, Y_linear_test, p_linear,
